...
type Report = { total: number; basis: string; price_table_version: string; by_model: Record<string, Breakdown>; by_project: Record<string, Breakdown>; by_provider: Record<string, Breakdown>; unpriced_models: Record<string, Tokens> };
const now = new Date();
...
function render(report: Report) {
  ...
  const hero = element("section"); hero.className = "hero"; hero.append(element("p", "Total estimado"), element("strong", money(report.total)), element("small", "Somente logs locais do Claude Code e Codex"));
  ...
  const grid = element("section"); grid.className = "grid"; grid.append(card("Por provedor", list(report.by_provider)), card("Por modelo", list(report.by_model)), card("Por projeto", list(report.by_project)));
...
async function load() { try { render(await invoke<Report>("current_month_report", { month })); } catch (error) { const app = document.querySelector<HTMLDivElement>("#app")!; const panel = element("main"); panel.className = "panel error"; const eyebrow = element("p", "GASTEI QUANTO?"); eyebrow.className = "eyebrow"; panel.append(eyebrow, element("h1", "Não foi possível ler os logs"), element("p", String(error)), element("small", "Confira se as pastas ~/.claude/projects e ~/.codex/sessions existem e tente novamente.")); app.replaceChildren(panel); } }
load();
